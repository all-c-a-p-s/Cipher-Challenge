...
var sampleText string = sample()
var sampleScore = scoreSample()
...
func mutate(keys *[]key, newGen int, genSize int, ciphertext string) {
	...
	for i := 0; i < genSize; i++ {
		for i := 0; i < newGen; i++ {
			...
			child.k[letters()[i1]], child.k[letters()[i2]] = child.k[letters()[i2]], child.k[letters()[i1]]

			plaintext := encipher(child.k, ciphertext)
			child.score = score(plaintext)
	...
	fmt.Println((*keys)[len(*keys)-1].score)
}
...
func geneticAlgorithm(genSize int, newGen int, maxGenerations int, ciphertext string) {
	...
	for {
		...
		mutate(&keys, newGen, genSize, ciphertext)
		gensElapsed++
	}

	//keys will already be sorted by selectFittest
  fmt.Println((keys)[len(keys)-1].score)
	fmt.Println(encipher(keys[len(keys)-1].k, ciphertext))
}
