...
fn decipher(text: &Vec<u8>, k: Key) -> Option<Vec<u8>> {
    ...
    for i in (0..text.len() - 1).step_by(2) {
        ...
        let i1: usize = k.a.iter().position(|&x| x == v[0])?;
        let i2: usize = k.b.iter().position(|&x| x == v[1])?;
...
fn dictionary_attack(ciphertext: &Vec<u8>) {
    ...
    for word in &dict {
        for word2 in &dict {
            let k: Key = Key {
                a: fill_grid(word.to_vec()),
                b: fill_grid(word2.to_vec()),
            };
...
fn renneal(ciphertext: &Vec<u8>, max_constant: i32, max_temp: f64, k: f64) -> String {
    ...
    loop {
        ...
        if solutions.contains(&p) {
            ...
            println!("Key 1: {}", String::from_utf8(res.key.a.to_vec()).unwrap());
            println!("Key 2: {}", String::from_utf8(res.key.b.to_vec()).unwrap());
            return p;
        }
        solutions.insert(p);
...
fn main() {
    ...
    let g1 = fill_grid(String::from("EUROPAN").into_bytes().to_vec());
    let g2 = fill_grid(String::from("EXPNSIV").into_bytes().to_vec());
